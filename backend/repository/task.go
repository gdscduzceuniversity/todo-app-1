...
func InsertTask(title, description string, completed int, dueDate time.Time) error {

	// Get the collection
	coll := db.Client.Database(databaseName).Collection(taskCollection)

	// Create a new task
	doc := models.Task{Title: title, Description: description, Completed: completed, CreateDate: time.Now(), DueDate: dueDate}
...
func ReadTask(id string) (models.Task, error) {
	...
	if err != nil {
		return models.Task{}, fmt.Errorf("error while getting task: %w", err)
	}
...
func ReadTasks(page, limit int64) ([]models.Task, int64, error) {
	// Get the collection
	coll := db.Client.Database(databaseName).Collection(taskCollection)
	...
	if err != nil {
		return []models.Task{}, 0, fmt.Errorf("error while getting tasks: %w", err)
	}
